...
import {targetAdienceList} from '../../../utils/utils'
import {useParams, useLocation, useSearchParams} from 'react-router-dom';
import {axiosInstance} from '../../../fetch/axiosConfig'

const DashboardSingleCourse = ({FetchedData, FetchedPlatforms, currentUser}) => {
...
const getPdfCourse = (e) => {
  setPdfCourse(e.target.files[0]);
}
...
const handleSaveCourse = async () => {
  ...
  try {
    const res = await axiosInstance.put(`/courses/${id}`, newPost);
    setContentData(res?.data?.data)
    setLoadingSave(false);
    toast.success('Upload Successfully!')

  } catch (error) {
    console.error('Error saving course:', error?.response?.data?.error || error);
    setLoadingSave(false);
  ...
  if (action === 'coursePDF' && !pdfCourse) {
    ...
    const formData = new FormData();
        formData.append('file', action === 'addcoverimage' ? imageCover : action === 'addcoursepdf' ? pdfCourse : action === 'upload-plat-image' ? platformImage : '');  // 'image' should match your backend field
        
    try {
      const res = await axiosInstance.put((action === 'upload-plat-image' || action === 'delete-plat-image' ) ? `/courses/${id}/platform/${platformId}/${action}` : `/courses/${id}/${action}`, formData, {
        headers: {
      ...
      if(action === 'addcoursepdf'){
        setLoadingSave(false);
        toast.success('Success upload!')
        return setPdfCourse(res?.data?.data)
      }
      if(action === 'deletecoursepdf'){
        setLoadingSave(false);
      ...
      console.error('Error handling course:', error?.response?.data?.error || error);
      setLoadingSave(false);
      toast.error(error?.response?.data?.error || error?.message)  
    }
  }
    ...
    const handlePromotion = async (itemId) => { 
      ...
      try {
        const res = await axiosInstance.put(`/courses/${itemId}/promotion`, newPromotion);
          setLoadingSave(false);
...
// }
    ...
    try {
      const res = await axiosInstance.put(`/courses/${id}/${method ? 'addchapter' : `chapter/${item_Id || elemId}/${action}`}`, (method || action === 'update') ? newChapter : "");
      if(method){
        setLoadingSave(false);
        setItem_Id('')
        setNewChapter({
      ...
      console.error('Error sadiing course:', error?.response?.data?.error || error);
      setLoadingSave(false);
      toast.error(error?.response?.data?.error || error?.message)
    }
  }
...
// }
  ...
  try {
    const res = await axiosInstance.put(`/courses/${id}/${method ? 'addplatform' : `platform/${item_Id || elemId}/${action}`}`, (method || action === 'update') ? newAffiliatePlats : "");
    if(method){
      setLoadingSave(false);
      setItem_Id('')
  ...
  useEffect(() => {
    ...
    setPdfCourse(FetchedData?.coursePDF)
  ...
  useEffect(() => {
    ...
    const imageOriginalName = FetchedPlatforms ? FetchedPlatforms.filter(item => item?.platformName === newAffiliatePlats.platformName)[0]?.imageOriginalName : ''
      ...
      newAffiliatePlats?.platformName === "Not Found" ? setNewAffiliatePlats({...newAffiliatePlats, platformListed: false, 
        courseLink: '', imaginarioCourseLink: '', imageName: undefined, imageOriginalName: undefined, allowsAffiliateLink: ""})
    : 
    setNewAffiliatePlats({...newAffiliatePlats, platformListed: true, courseLink: link, imaginarioCourseLink: link, 
    ...
    <div className='create-book-container' >
     ...
     {/* d-flex justify-content-between align-items-center */}
      ...
      <div className='action-btn mx-1'>
        <Link to={`/imaginario-spaces/${currentUser?._id}/course/new-course?languageTrackId=7478347847383874`} type='button' className=''>+Language</Link>
      </div>
   </div>

   <div className='add-link-div'>
        <button type='button' className='add-link mx-1' onClick={handleSaveCourse} >Save</button>
        <button type='button' className='add-link' onClick={() => handleCancel('courseInfo')}>Cancel</button>
      </div>
  </div>
      ...
      <div className='inputs'>
        ...
        <SelectsVideo text={'Levels'} inputId={'level'}
        selectItems={["Begineer","Intermediate","Advance", "All Levels"]} newPost={newPost} getPost={getPost}/>
      ...
      <div >
      <TextAreasVideo text={'Course Description'} inputId={'desc'} newPost={newPost} getPost={getPost} editorData={editorData} setEditorData={setEditorData}/>
      <TextAreasVideo text={'Tags'} inputId={'tags'} newPost={newPost} getPost={getPost}/>
      </div>

    {/* UPLOAD COVER */}
    <div className='sections'>
      <h4 className='m-0 my-3 text-dark'>Course Cover Image</h4>
      ...
      {/* UPLOAD COVER */}
    <div className='sections'>
      <h4 className='m-0 my-3 text-dark'>Course PDF</h4>
      <div className='div-border'>
        <div className='inputs'>
        <UploadCoverImg text={'Upload PDF file'} inputId={'name'} list={contentData} newPost={pdfCourse} getPost={getPdfCourse}/>
        </div>
        <div className='add-link-div'>
        <button type='button' className='add-link mx-1' onClick={() => handleFileUpload('addcoursepdf')} disabled={!pdfCourse?.name}>Upload</button>
        <button type='button' className='add-link' onClick={() => handleFileUpload('deletecoursepdf')} disabled={!pdfCourse?.name}>Delete</button> 
        </div>
        </div>
      </div>
      ...
      {/* BOOK CHAPTERS */}
      <div className='sections  '>
      <h4 className='m-0 my-3 text-dark'>Course Chapters</h4>
      <div className='div-border'>
      ...
      <div className='inputs5'> 
        <SelectsVideo text={'Platform Name'} inputId={'platformName'}
          ...
          <InputsVideo text={'Course Link'} inputId={'courseLink'} change={'platform'} newPost={newAffiliatePlats} getPost={getNewAffiliatePlats}/>
          {currentUser?.role === "admin" && <InputsVideo text={'Imaginario Link'} inputId={'imaginarioCourseLink'} change={'platform'} newPost={newAffiliatePlats} getPost={getNewAffiliatePlats}/>}
          <div className='add-link-div'>
      ...
      <div className='inputs6'> 
         ...
         <InputDateTimeVideo text={'End'} inputId={'displayPeriodEnd'} change={'promotion'} newPost={newPromotion} getPost={getNewPromotion}/>
          ...
          selectItems={sellingPlatData?.map(item => [item.platformName !== 'Not Found' ? item.platformName : item.platformNameNotListed ])} change={'promotion'} 
          newPost={newPromotion} getPost={getNewPromotion} multipleOption={true}/>
