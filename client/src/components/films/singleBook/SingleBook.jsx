...
const SingleBook = ({list, setUrl, imaUserId, imaUserContent, fetchedContentList, trackProfile, TrackName, previewBtns}) => {
  const {currentUser} = UseUserContext()

const From_the_same_author = fetchedContentList?.filter(item => item?.author === list?.author && item?._id !== list?._id)
const Similar = fetchedContentList?.filter(item => list?.genre?.some((i) => item?.genre?.includes(i)) && item?._id !== list._id)?.sort((a, b) => a?.title - b?.title)
// console.log(Similar) 
return (
    <Wrapper>
        <div className='singleBook-container'>
            ...
            {fetchedContentList && list ? From_the_same_author.length > 0 &&
             ...
             {fetchedContentList && list ?  Similar.length > 0 && 
